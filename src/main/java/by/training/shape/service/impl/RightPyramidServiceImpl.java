...
public class RightPyramidServiceImpl implements PyramidService {
    ...
    public double triangleSideLength(RightPyramid pyramid){
        double result;
        result = Math.sqrt(Math.pow(pyramid.getS().getX() - pyramid.getA().getX(), 2) + Math.pow(pyramid.getS().getY() - pyramid.getA().getY(), 2)
                + Math.pow(pyramid.getS().getZ() - pyramid.getA().getZ(), 2));
        return result;
    }
